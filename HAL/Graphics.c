...
GFX GFX_construct(uint32_t defaultForeground, uint32_t defaultBackground)
{
    ...
    Graphics_initContext(&gfx.context, &g_sCrystalfontz128x128,
...
void GFX_print(GFX *gfx_p, char *string, int row, int col)
{
    ...
    Graphics_drawString(&gfx_p->context, (int8_t*) string, -1, xPosition,
                        yPosition, OPAQUE_TEXT);
}
...
void GFX_fillRectangle(GFX *gfx_p, int minX, int minY, int maxX, int maxY)
{
    Graphics_Rectangle myRect = { minX, minY, maxX, maxY };
    Graphics_fillRectangle(&gfx_p->context, &myRect);
}

void GFX_drawRectangle(GFX *gfx_p, int minX, int minY, int maxX, int maxY)
{
    Graphics_Rectangle myRect = { minX, minY, maxX, maxY };
    Graphics_drawRectangle(&gfx_p->context, &myRect);
}
